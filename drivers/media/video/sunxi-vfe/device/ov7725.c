/*
* A V4L2 driver for ov7725 cameras.
*
*/
...
#define CAP_BDG 0
#if(CAP_BDG == 1)
#define vfe_dev_cap_dbg(x,arg...) printk("[OV7725_CAP_DBG]"x,##arg)
#else
...
static struct regval_list sensor_default_regs[] = {
    ...
    //0x11, 0x03,
#if (VI_FRAME_RATE == 60)
    {0x11, 0x00}, // wangxin 20110531, 00/01/03/07 for 60/30/15/7.5fps
#elif (VI_FRAME_RATE == 30)
    {0x11, 0x01}, // wangxin 20110621, 00/01/03/07 for 60/30/15/7.5fps
    {0x0d, 0x41},
    {0x2a, 0x00},
    {0x2b, 0x00},
    {0x33, 0x00},
...
#if (VI_FRAME_RATE == 60)
    {0x22, 0xff}, // wangxin 20110531, ff/7f/3f/1f for 60/30/15/7.5fps
    {0x23, 0x01}, // wangxin 20110531, 01/03/07/0f for 60/30/15/7.5fps
#elif (VI_FRAME_RATE == 30)
    {0x22, 0x7f}, // wangxin 20110531, ff/7f/3f/1f for 60/30/15/7.5fps
    {0x23, 0x03}, // wangxin 20110531, 01/03/07/0f for 60/30/15/7.5fps
#else
    {0x22, 0x3f},//banding
    {0x23, 0x07},
#endif

    {0x24, 0x40},// AGC/AEC target
    {0x25, 0x30},
    ...
    */
    ...
    // len correction for turly second
    {0x47 ,0x8a},
    {0x48 ,0x16},
    {0x49 ,0x10},
    {0x4a ,0x00},
...
static int sensor_read8(struct v4l2_subdev *sd, unsigned char reg,
	unsigned char *value)
{
	int ret=0;
	int cnt=0;
...
static int sensor_write8(struct v4l2_subdev *sd, unsigned char reg,
    unsigned char value)
{
	int ret=0;
	int cnt=0;
...
static int sensor_write_array(struct v4l2_subdev *sd, struct regval_list *regs, int array_size)
{
	...
	while(i<array_size)
	{
		if(regs->addr == REG_DLY) {
			...
			//printk("write 0x%x=0x%x\n", regs->addr, regs->data);
			sensor_write8(sd, regs->addr, regs->data);
		}
		i++;
		regs++;
	}
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	unsigned char rdval;
	printk("rock write ++");
        LOG_ERR_RET(sensor_write8(sd, 0x12, 0x80));
	printk("rock write --");
        


	LOG_ERR_RET(sensor_read8(sd, 0x0a, &rdval))
	printk(KERN_ERR"****wyb %s:%d/%s()! reg:0x0a=0x%02x\n", __FILE__, __LINE__, __func__, rdval);
        
	if(rdval != 0x77)
		return -ENODEV;
        
	LOG_ERR_RET(sensor_read8(sd, 0x0b, &rdval))
	
	printk(KERN_ERR"****wyb %s:%d/%s()! reg:0x0b=0x%02x 2016/7/19 11:14\n", __FILE__, __LINE__, __func__, rdval);
...
//mbus_code=0x00002008, fmt->code=0x0000200c
//mbus_code=0x00002009, fmt->code=0x0000200c
//mbus_code=0x00002006, fmt->code=0x0000200c xxxx
//mbus_code=0x00002007, fmt->code=0x0000200c
	...
	enum v4l2_mbus_pixelcode *code)
{
	if (index >= N_FMTS)
	{
		printk(KERN_ERR"****wyb %s:%d/%s()! index=%d, N_FMTS=%d, *code=0x%08x, mbus_code=0x%08x\n", __FILE__, __LINE__, __func__, index, N_FMTS, *code, sensor_formats[index].mbus_code);
	...
	struct sensor_win_size **ret_wsize)
{
	...
	{
		printk(KERN_ERR"****wyb %s:%d/%s()! index=%d, N_FMTS=%d, mbus_code=0x%08x, fmt->code=0x%08x\n", __FILE__, __LINE__, __func__, index, N_FMTS, sensor_formats[index].mbus_code, fmt->code);
