...
static struct i2c_client *bmg_client = NULL;
...
static int gsensor_detect(struct i2c_client *client, struct i2c_board_info *info)
{
    ...
    printk("bmg160 %s enter \n",__func__);
    if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)){
        printk("i2c usage wrong \n");
	return -ENODEV;
    } 

    if (twi_id == adapter->nr) {
        client->addr = BMG160_I2C_ADDR;
	//printk("ignore i2c read to teset first write \n");
        printk(KERN_DEBUG "%s:addr=0x%x , r16 i2c bus %d \n", __func__, client->addr, twi_id );	
	ret = i2c_smbus_read_byte_data(client,BMG160_CHIP_ID_REG);
        printk(KERN_DEBUG "Read chip id value 0x%x[%d] \n", ret, ret);	
	//strlcpy(info->type, SENSOR_NAME, I2C_NAME_SIZE);
	//return 0;

        if ((ret & 0x00FF) == SENSOR_CHIP_ID_BMG){
	    printk(KERN_DEBUG "Bosch Sensor bmi055-bmg160 detected \n");	
	    strlcpy(info->type, SENSOR_NAME, I2C_NAME_SIZE);
	    return 0; 
	} else {
	    return -ENODEV;   
		...
		u8 *data, u8 len)
{
	...
	u8 buffer[2];
#endif
        printk("%s , i2c addr %x, reg %x, len %d \n", __func__, client->addr, reg_addr, len);
	if (NULL == client)
		return -EPERM;

	while (0 != len--) {
#ifdef BMG_SMBUS
		printk("call i2c_smbus_write\n");
		dummy = i2c_smbus_write_byte_data(client, reg_addr, *data);
		printk("call i2c_smbus_write return %x\n", dummy);
#else
		buffer[0] = reg_addr;
		buffer[1] = *data;
		...
		data++;
		if (dummy < 0) {
			dev_err(&client->dev, "ignore error writing i2c bus");
			//return -EPERM;
		}
...
static int bmg_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "probe function entrance");
...
*/
	...
	printk("bmg160 :do not set chip to suspend mode \n");	
#ifdef CONFIG_HAS_EARLYSUSPEND
